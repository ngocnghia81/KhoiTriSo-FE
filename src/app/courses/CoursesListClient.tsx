...
export default function CoursesListClient({ initialCourses, initialPagination }: CoursesListClientProps) {
  ...
  const isDefaultState = !selectedCategory && !selectedLevel && !searchTerm && page === 1 && sortBy === 'totalStudents_desc';
  
  // Use initialCourses if in default state and available, otherwise fetch
  const shouldUseInitial = isDefaultState && initialCourses && initialCourses.length > 0;
  
  const { courses: fetchedCourses, loading, error, pagination: fetchedPagination } = useCourses({
  ...
  }, { enabled: !shouldUseInitial }); // Disable fetch if using initial data
