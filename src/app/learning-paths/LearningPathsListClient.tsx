...
interface LearningPathsListClientProps {
  initialPaths?: LearningPath[];
  initialPagination?: PaginationInfo;
}

export default function LearningPathsListClient({ initialPaths, initialPagination }: LearningPathsListClientProps) {
  ...
  const isDefaultState = !categoryFilter && levelFilter === 'all' && sortBy === 'createdAt' && sortOrder === 'desc';
  
  // Use initialPaths if in default state and available, otherwise fetch
  const shouldUseInitial = isDefaultState && initialPaths && initialPaths.length > 0;

  useEffect(() => {
    // Only fetch if not using initial data (filters changed from default)
    if (!shouldUseInitial) {
      fetchLearningPaths();
    }
